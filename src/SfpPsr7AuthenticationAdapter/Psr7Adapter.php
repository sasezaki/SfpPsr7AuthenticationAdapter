...
namespace SfpPsr7AuthenticationAdapter;
 ...
 */
class Psr7Adapter implements AdapterInterface
{
    /**
    ...
    public function authenticate()
    {
        ...
        if (!$this->request->hasHeader($getHeader)) {
            return $this->challengeClient();
        }
        $authHeaders = $this->request->getHeader($getHeader);
        $authHeader = current($authHeaders);
        if (!$authHeader) {
            return $this->challengeClient();
        }
    ...
    public function challengeClient()
    {
        ...
        if (in_array('basic', $this->acceptSchemes)) {
            $this->response = $this->response->withHeader($headerName, $this->_basicHeader());
        }
        if (in_array('digest', $this->acceptSchemes)) {
            $this->response = $this->response->withHeader($headerName, $this->_digestHeader());
        }
        return new Authentication\Result(
    ...
    protected function _parseDigestAuth($header)
    {
        ...
        $cUriPath = parse_url($temp[1], PHP_URL_PATH);
